...
export class CatFoodService {
  ...
  update(catFood: CatFood): Observable<CatFood> {
    const url = `${this.catFoodUrl}/${catFood.catid}&${catFood.foodid}`;
  ...
  add(catFood: CatFood): Observable<any> {
    return this.httpClient.post<any>(this.catFoodUrl, catFood).pipe(
      catchError(this.handleError<any>('addCatFood'))
    );
  }

  delete(catFoodId: number): Observable<any> {
    const url = `${this.catFoodUrl}/${catFoodId}`;
    console.log(url);
