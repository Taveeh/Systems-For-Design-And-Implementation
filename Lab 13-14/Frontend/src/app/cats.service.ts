...
export class CatsService {
  ...
  update(cat: Cat): Observable<Cat> {
    ...
    return this.httpClient.put<Cat>(url, {name: cat.name, breed: cat.breed, catYears: cat.catYears}, this.httpOptions).pipe(
      catchError(this.handleError<Cat>('updateCat'))
    );
  }

  add(cat: Cat): Observable<any> {
    return this.httpClient.post<any>(this.catsUrl, {name: cat.name, breed: cat.breed, catYears: cat.catYears}, this.httpOptions).pipe(
