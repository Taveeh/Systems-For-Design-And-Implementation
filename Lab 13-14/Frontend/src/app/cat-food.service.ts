...
export class CatFoodService {
  ...
  getCatFoods(): Observable<any> {
    return this.httpClient.get<Array<CatFood>>(this.catFoodUrl).pipe(
      catchError(this.handleError<CatFood[]>('getCatFoods', []))
    );
  }
  ...
  update(catFood: CatFood): Observable<CatFood> {
    const url = `${this.catFoodUrl}/${catFood.cat}&${catFood.food}`;
  ...
  add(catFood: CatFood): Observable<any> {
    console.log(catFood);
    return this.httpClient.post<any>(this.catFoodUrl, {catId: catFood.cat, foodId: catFood.food}).pipe(
