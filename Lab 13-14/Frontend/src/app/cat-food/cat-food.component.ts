...
export class CatFoodComponent implements OnInit {
    ...
    addCatFood() {
        this.catFoodService.add({cat: this.selectedCat.id, food: this.selectedFood.id} as CatFood).subscribe(
            _ => this.getCatFood()
        );
    }
    ...
    delete(catFood: CatFood) {
        this.catFoodService.delete(catFood.cat, catFood.food).subscribe(
            _ => this.getCatFood()
        );
    }
    ...
    getCatFood() {
        this.catFoodService.getCatFoods().subscribe(
            catFoods => {
                this.catFoods = catFoods.catFoods;
